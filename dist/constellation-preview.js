...
export function constellationChart(lines, selected, width=270, height=190) {
  const vertices = [...new Map(lines.flat().map(p=>[p.join(','),p])).values()];
  const reference = vertices.length ? vertices : [selected];
  ...
  const projected = [...vertices,selected].map(project).filter(Boolean);
  ...
  return {width,height,scale,segments,stars:vertices.map(map).filter(Boolean),selected:map(selected),project};
}
