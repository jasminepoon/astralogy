...
export function syncControls({interaction,view:v,delegated,planning,request,pending,accepted,quote,editing,error}){
 button($('#brush-mode'),quote?.kind.startsWith('gravity-')?'Shape gravity':'Preview a push',{icon:'Cursor',variant:'ghost'});
 ...
 button($('#delegate'),arrived?'Home, verified':active||busy?workLabel:'Take the helm',{variant:active||busy||arrived?'outline':'primary',icon:arrived?'Check':active||busy?'SpinnerGap':delegated?'Play':'Sparkle',busy:active||busy,disabled:active||busy||arrived});
 button($('#interrupt'),'Interrupt',{icon:'Pause',variant:'outline',disabled:!active&&!busy&&!planning});
 $('#driver-status').textContent=arrived?'Arrival verified.':pending?'Revising your quote…':busy?'Live Astra is considering…':editing?'You are shaping the field.':!active&&delegated?(error?'Paused · needs a change':'Paused · you have the helm'):active?(delegated?'Astra flying':'Flying your route'):v.mode==='plan'?'Planning':'Drive · gestures apply';
 ...
 $('#route-question').hidden=!quote?.kind.startsWith('gravity-')||active||busy;$('#delegate').hidden=!v.calibration||arrived||['mined','at-star'].includes(v.phase);$('#delegation-note').hidden=!v.calibration||arrived;$('#budget-panel').hidden=arrived;button($('#chat-form button[type=submit]'),'',{icon:busy?'SpinnerGap':'ArrowUp',variant:'ghost',busy,disabled:busy||arrived});$('#message').disabled=arrived;$('#skip-stop').disabled=arrived;$('#skip-stop').hidden=v.objective!=='home'||!v.report||!v.deposit?.fuel||!!v.target||arrived;$('#less-fuel').hidden=!quote?.kind.startsWith('gravity-');
 ...
 $('#journey-map').setAttribute('aria-label',arrived?'Verified arrival. Open Browse constellations to explore the sky.':interaction==='look'?'Move view. Drag to rotate the camera; this does not select or push.':interaction==='select'&&!quote?.kind.startsWith('gravity-')?'Select a catalogue light. Ringed bright lights are suggested targets; hover any catalogue star to highlight it. Selection is free.':v.target?'Ship in computed flight. Use Move view to rotate the camera.':quote?.kind.startsWith('gravity-')?'Gravity proposal. Drag the separate glowing Bend gravity handle to change the field, route and cost. Release follows the selected Plan or Drive mode.':'Preview a push. Drag outward from the ship; direction and length set a free impulse preview. Move view rotates the camera.');
 $('#gravity-handle').setAttribute('aria-label',`Bend gravity. Drag to change the route. ${v.mode==='plan'?'Release keeps a free preview.':'Release applies the displayed quote.'}`);$('#gravity-handle').title=v.mode==='plan'?'Drag to change the route; release keeps preview':'Drag to change the route; release applies';$('#gravity-handle').setAttribute('aria-busy',String(document.body.classList.contains('handle-busy')));
}
