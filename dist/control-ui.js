...
export function actionButton(el,label){
 ...
 button(el,label,{variant:'outline',icon:fuel?'SlidersHorizontal':/Apply/.test(label)?'ArrowBendUpRight':/Advance/.test(label)?'ArrowRight':/receipt/.test(label)?'List':undefined});
}
export function syncControls({view:v,delegated,request,pending,accepted,quote,editing,error}){
 const arrived=v.phase==='arrived',active=v.active,busy=Boolean(request);
 ...
 const workLabel=pending?'Revising the route':busy?(v.calibration?'Considering the route':'Finding our position'):v.target?'Following the route':quote?.kind.startsWith('gravity-')?'Field preview · Astra driving':'Astra has the helm';
 button($('#delegate'),arrived?'Home, verified':active||busy?workLabel:delegated?'Resume journey':'Get us home. You drive.',{variant:active||busy||arrived?'outline':'primary',icon:arrived?'Check':active||busy?'SpinnerGap':delegated?'Play':'Sparkle',busy:active||busy,disabled:active||busy||arrived});
 button($('#interrupt'),'Interrupt',{icon:'Pause',variant:'outline',disabled:!active&&!busy});
