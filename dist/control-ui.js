...
export function syncControls({interaction,view:v,delegated,planning,request,pending,accepted,quote,editing,error}){
 ...
 $('#route-question').hidden=!quote?.kind.startsWith('gravity-')||active||busy;$('#delegate').hidden=!v.calibration||arrived;$('#delegation-note').hidden=!v.calibration||arrived;$('#budget-panel').hidden=arrived;$('#chat-form button[type=submit]').disabled=busy||arrived;$('#message').disabled=arrived;$('#skip-stop').disabled=arrived;$('#skip-stop').hidden=!v.report||!!v.target||arrived;$('#less-fuel').hidden=!quote?.kind.startsWith('gravity-');
